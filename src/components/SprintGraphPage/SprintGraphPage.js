...
class SprintGraphPage extends Component {
  ...
  constructTimeSpent = () => {
    ...
    timeSpentLogs.forEach(log => {
      ...
      timeSpentData.push({ date: timestamp, hours: total, name: "actual" });
    });
  ...
  constructProjectedTimeSpent = () => {
    ...
    projection.push({ date: nextMonday, hours: 45, name: "projection" });
  ...
  constructProjectedTimeRemaining = () => {
    ...
    nextMonday.setDate(startDate.getDate() + 7);
    nextMonday.setHours(startDate.getHours() - 4);
    projection.push({ date: nextMonday, hours: 0, name: "projection" });
  ...
  render() {
    ...
    const data = [timeSpentProjection, timeSpentData];
    ...
    return (
      <div>
        <Chart theme={theme}>
          ...
          <Lines data={data} x="date" y="hours" stroke="name" />
        </Chart>
        <Chart theme={theme}>
          <Lines data={timeRemainingProjection} x="date" y="hours" />
          <Points data={timeRemainingData} x="date" y="hours" />
        </Chart>
      </div>
    );
  }
